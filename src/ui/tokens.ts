...
/**
 * 어떤 표면 위에서도 보이는 반투명 흰 오버레이. 치지직 실측 `--color-bg-overlay-*` 계열이다
 * (`#ffffff4d` · `#fff3` · `#ffffff1a`).
 *
 * 🔴 **끈 상태의 스위치 트랙에 이걸 쓴다.** 불투명 회색(`BORDER.subtle` = `#24272b`)을 쓰면
 * 패널 배경(`#2e3033`)과 거의 같은 밝기라 **트랙이 있는지조차 안 보인다** — 켜짐/꺼짐 구분이
 * 안 된다는 보고가 여기서 나왔다 (2026-08-21). 흰색 알파는 어떤 배경에서도 한 단 밝게 뜬다.
 */
export const OVERLAY = {
  /** 30% 흰색 — 끈 스위치 트랙처럼 "있다는 것이 보여야" 하는 면 */
  strong: token('--color-bg-overlay-01', 'rgba(255, 255, 255, 0.3)'),
  /** 20% 흰색 */
  base: token('--color-bg-overlay-02', 'rgba(255, 255, 255, 0.2)'),
  /** 10% 흰색 — 옅은 구분선 */
  weak: token('--color-bg-overlay-03', 'rgba(255, 255, 255, 0.1)'),
} as const;
