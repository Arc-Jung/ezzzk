...
export const adBlockNoticeFeature: Feature = {
  ...
  start: (ctx) => {
     ...
     *
     * 🔶 미검증(정황상 유력, 2026-08-21 `docs/widescreen-debug-2026-08-21.md`): 이 모달이
     * 오래 떠 있을수록 뷰모드 버튼의 aria-label 판정 불가 상태와 겹칠 여지가 커진다.
     * 원인을 재현하지는 못했지만(6차례 시도 모두 실패), 모달이 화면에 머무는 시간을
     * 줄이는 것 자체는 부작용 없는 증상 완화라 먼저 적용한다 — 2회에서 1회로 줄이면
     * 숨김까지의 시간이 최대 절반(≈2.4s → ≈1.2s)으로 준다.
     */
    const MAX_CLICK_ATTEMPTS = 1;
