...
| 멀티뷰 구성 시트 (BETA) — 조작은 상단, 인기 방송 10개씩 추가 로드 | ![멀티뷰 구성](screenshots/demo-laptop13-multiview-config.png) |
| 멀티뷰 시청 (BETA) — 4분할, 초점 슬롯(사이드 채팅 대상·화질 우선순위)에 초록 테두리 | ![멀티뷰 시청](screenshots/demo-laptop13-multiview.png) |
...
> ℹ️ `멀티` 버튼은 **이전에 보던 구성이 있으면 그대로 이어서 연다.** 없을 때만 구성 시트가 뜬다
> (구성을 바꾸려면 스테이지의 `구성`). **슬롯·사이드 채팅 기능은 임시로 껐다** — 닉네임만 두 번 찍히고
> 본문이 안 보이는 등 버그가 여러 건 있어 설정에서 아예 선택할 수 없게 막았다. 재작업 뒤 되살릴 예정이다.
>
> ℹ️ 멀티뷰는 아직 **베타**다. 진입점(`멀티` 버튼·구성 시트 제목·스테이지 조작 바) 세 곳에 `BETA` 를 표기한다.
> 2026-08-16 까지 슬롯마다 치지직 플레이어 컨텍스트 메뉴가 좌상단에 새어 나와 멀티뷰 시청 화면을
> 표에서 뺐었는데, 되살리기 CSS 를 고친 뒤 3프로필 전부 노출 0건이라 다시 넣었다
> (`capture-demo.mjs` 가 노출을 발견하면 저장하지 않고 다른 채널로 다시 찍는다).
...
| FR-19 | **음량 평탄화(오디오 컴프레서)** | 기본 꺼짐. 방송마다 다른 음량을 눌러 균일하게. 구현은 chzzk-plus(MIT) 참조 |
...
| FR-14 | 2~4 화면 분할 동시 시청(멀티뷰) | **모든 슬롯이 소리를 낸다** (2026-08-20 변경 — 슬롯마다 음소거·볼륨 컨트롤이 있어 직접 조절한다). 슬롯·사이드 채팅은 버그로 임시 비활성. 조작 바(`− + 50% 구성 ⛶ 해제`)는 **가운데 상단**, 볼륨은 마스터다 |
...
> 그 상태에서 가로로 돌리면 FR-10 이 자동 적용된다 (915×412 실측: 채팅 183px, 영상 731×411, 여백 0).
>
> ⚠️ **확장이 데스크톱 사이트로 자동 전환해 주지는 못한다.** 모바일 UA 로 `chzzk.naver.com` 에
> 접속하면 치지직이 다시 `m.chzzk` 로 보내기 때문에, 확장이 리다이렉트하면 무한 루프가 된다.
> 억제 수단(쿼리 파라미터 8종·쿠키 4종)을 실측으로 전부 시도해 **12/12 실패**를 확인했다
> (실측 근거 `chzzk-dom-33` 은 개발 저장소의 로컬 덤프에 있다).
> 유일한 방법인 UA 위조는 광범위 권한이 필요해 채택하지 않았다 — 대신 **안내를 1회 노출**한다.
...
- ⭐ **"데스크톱 사이트 요청"을 켠다.** 켜지 않으면 `m.chzzk.naver.com` 으로 리다이렉트되는데,
  ...
  (915×412 실측: 채팅 183px, 영상 731×411, 여백 0).
- 태블릿도 마찬가지다. **태블릿 UA 역시 `m.chzzk` 로 리다이렉트되는 것을 실측으로 확인**했다.
...
- **2026-08-23** — 초점 슬롯의 초록 테두리 강조 표시 제거(버튼 상태 자체는 유지), 초광폭 화면(비율 1.8 이상 — 21:9·32:9 데스크톱 모니터부터 맥시마이즈한 노트북·태블릿 가로까지 모두 해당)에서 영상을 왼쪽에 붙일지 가운데(치지직 기본값)에 둘지 선택하는 옵션 추가(전체화면 여부와 무관하게 적용), **멀티뷰 슬롯의 자동 화질 변경·자동 음소거 해제가 먹통이던 문제 수정** — 화질은 설정 메뉴를 열어야 하는 경로에서 재시도가 전혀 없었고 목표 항목을 찾고도 합성 클릭이 반영되지 않았음(키보드 Enter 활성화로 교체), 음소거는 2026-08-20 정책 변경 때 등록 시 자동 해제 지시가 함께 사라져 자동재생 정책으로 음소거된 슬롯을 아무도 풀어주지 않았음(등록 시 1회 해제 복원)
- **2026-08-22** — 데모 스크린샷 갱신, 광고 차단 안내 재시도 로직 정리, **멀티뷰 사이드/슬롯 채팅 기능 임시 비활성화**(버그 다수 발견, 재작업 예정 — 설정에서 선택할 수 없고 저장값과 무관하게 표시되지 않는다), 멀티뷰 진입 클릭을 원본 재생 조작으로 오인해 원본 소리가 겹치던 문제 수정, 초점 슬롯에 초록 테두리 표시 복원(초점 버튼이 먹통으로 보이던 문제), 모바일 세로 2분할에서 슬롯 높이의 약 45%가 검은 여백이던 레이아웃 수정
- **2026-08-21** — 볼륨 컨트롤에 라우드니스 컴프레서 토글 추가, 채팅 폭 조절·프리셋 UI를 플레이어 우측 중앙으로 재배치, 멀티뷰 오디오 라우팅 제거, 남은 텍스트/글리프 아이콘을 공용 SVG로 교체, 라이선스 고지를 설정 탭으로 이동, 팝업 스크롤·접근성 보강
- **2026-08-20** — ezzzk 최초 공개 릴리스 (기본 시청 화면 정리, 설정 패널, 멀티뷰 베타, 클린봇 기본 해제 등)
