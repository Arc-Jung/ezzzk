...
- **2026-08-23** — 초점 슬롯의 초록 테두리 강조 표시 제거(버튼 상태 자체는 유지), 초광폭 화면(비율 1.8 이상 — 21:9·32:9 데스크톱 모니터부터 맥시마이즈한 노트북·태블릿 가로까지 모두 해당)에서 영상을 왼쪽에 붙일지 가운데(치지직 기본값)에 둘지 선택하는 옵션 추가(전체화면 여부와 무관하게 적용)
