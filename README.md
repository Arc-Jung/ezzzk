...
| —       | 치지직 내장 클린봇 자동 끄기                           | **기본 켜기.** 계정 설정의 클린봇 토글 자체를 끈다(자체 필터를 만드는 것이 아니다). 이미 꺼져 있으면 건드리지 않는다                                                                                                    |
...
- **모바일에서 볼륨 컨트롤(`−` / % / `+` / 평탄화)을 컨트롤바 바로 위 전용 줄로 옮겼다** — 일시정지 버튼 위, 왼쪽 정렬. 예전에는 우측 버튼 그룹에 같이 들어가 있었는데 폰 화면에는 그만한 가로 공간이 없어(실측 2026-09-03: 세로 412×915 는 그룹 폭 270px 에 필요 폭 414px, 가로 915×412 는 308px 에 450px) 줄바꿈으로 버튼이 서로 밀리고 볼륨 컨트롤이 보이지 않거나 다른 버튼과 겹쳤다. 태블릿·노트북·데스크톱 배치는 그대로다. 대신 모바일에서는 컨트롤바가 한 줄(44px) 높아진다
