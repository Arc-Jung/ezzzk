...
- **멀티뷰 중 사라졌던 볼륨·컴프레서 버튼 복원** — 마스터 볼륨은 스테이지 바에 이미 있었지만 음량 평탄화(컴프레서)는 대응하는 버튼 자체가 없었고, 애초에 슬롯 오디오에는 컴프레서 그래프가 아예 연결돼 있지 않았다. 스테이지 조작 바에 컴프레서 토글을 추가하고, 슬롯 iframe 쪽에도 저장값을 구독해 실제로 컴프레서가 걸리도록 오디오 그래프를 연결
