...
| 멀티뷰 시청 (BETA) — 4분할, 소리 나는 슬롯에 초록 테두리 | ![멀티뷰 시청](screenshots/demo-laptop13-multiview.png) |
...
> (구성을 바꾸려면 스테이지의 `구성`). 사이드 채팅(BETA)은 무대가 좁으면 자동으로 접히고,
> 데모 화면은 채팅을 끈 상태로 찍었다 — 슬롯 채팅 수집이 실사이트에서 비는 문제를 먼저 해결해야 한다
> (조사 기록은 개발 저장소의 로컬 문서에 있다).
>
...
| FR-14 | 2~4 화면 분할 동시 시청(멀티뷰) | **모든 슬롯이 소리를 낸다** (2026-08-20 변경 — 슬롯마다 음소거·볼륨 컨트롤이 있어 직접 조절한다). 사이드 채팅은 활성 슬롯 1개. 조작 바(`− + 50% 구성 ⛶ 해제`)는 **가운데 상단**, 볼륨은 마스터다 |
...
- **2026-08-22** — 데모 스크린샷 갱신, 광고 차단 안내 재시도 로직 정리
